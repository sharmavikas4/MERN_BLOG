...
function Edit(props) {
    ...
    function del(pid, id) {
      setIsDeleting(true);
      axios.post(`${import.meta.env.VITE_REACT_APP_SERVER_URL}/del`, { pid, id, ...post }, { withCredentials: true })
        .then((res) => {
          if (res.data.message) {
            setIsDeleting(false);
            navigate("/dashboard");
    ...
    function ed(pid, id) {
      setIsEditing(true);
      axios.post(`${import.meta.env.VITE_REACT_APP_SERVER_URL}/edit`, { pid, id, title: post.title, content: post.content, image: file }, {
        ...
        <div className="edit">
          <Card>
            <form onSubmit={(event) => {
              ...
              <textarea rows="8" cols="50" name="content" onChange={change} value={post.content}></textarea>
...
// Add prop validation for `logout`
Edit.propTypes = {
